...
export function BookmarksModal({ onClose }: BookmarksModalProps) {
  ...
  const renderBookmarkRow = (bookmark: BookmarkItem, indented = false) => (
    <button
      key={bookmark.id}
      className="flex items-center gap-[8px] px-[10px] h-[36px] rounded-[8px] cursor-pointer group w-full text-left hover:bg-secondary/30"
    >
  ...
  const renderFolderRow = (bookmark: BookmarkItem) => {
    ...
    return (
      <div key={bookmark.id}>
        <button
          className="flex items-center gap-[6px] px-[10px] h-[38px] rounded-[8px] cursor-pointer group w-full text-left hover:bg-secondary/30"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div
        ref={modalRef}
        className="relative bg-card rounded-[14px] w-full max-w-[520px] shadow-[0px_20px_60px_0px_rgba(0,0,0,0.3)] flex flex-col h-[700px] max-h-[90vh]"
        role="dialog"
        aria-modal="true"
        aria-labelledby="bookmarks-title"
      >
        {/* Header */}
        <div className="flex items-center gap-[12px] px-[20px] h-[54px] shrink-0 border-b-2 border-border">
          <GripVertical className="size-[20px] text-foreground opacity-40 cursor-grab shrink-0" strokeWidth={2} />
          <h2 id="bookmarks-title" className="text-foreground text-[24px] font-bold flex-1 leading-[36px]">
            Bookmarks
          </h2>
          <button
            onClick={onClose}
            className="text-foreground hover:opacity-60 transition-opacity shrink-0 cursor-pointer"
        ...
        {/* Tabs */}
        <div className="flex shrink-0 border-b-2 border-border px-[20px]" role="tablist">
          <button
            onClick={() => setActiveTab('my')}
            className={`flex-1 h-[44px] text-[14px] text-center border-b-2 -mb-[2px] cursor-pointer transition-colors ${
              activeTab === 'my'
        ...
        {/* Search + Actions */}
        <div className="flex items-center gap-[8px] h-[40px] shrink-0 px-[20px] mt-[16px]">
          ...
          <button
            className="flex items-center justify-center shrink-0 size-[40px] rounded-[8px] bg-secondary shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_0px_rgba(0,0,0,0.1)] cursor-pointer hover:opacity-80"
            aria-label="Create new folder"
          >
            <FolderClosed className="size-[18px] text-foreground" strokeWidth={1.5} />
          </button>
          <button
            className="flex items-center justify-center shrink-0 size-[40px] rounded-[8px] bg-primary shadow-[0px_1px_3px_0px_rgba(0,0,0,0.1),0px_1px_2px_0px_rgba(0,0,0,0.1)] cursor-pointer hover:opacity-80"
        ...
        <div className="flex-1 min-h-0 px-[20px] mt-[16px] mb-[20px]">
